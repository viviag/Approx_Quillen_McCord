...
For comparison, we can consider a more standard construction.

\begin{proposition}
  \label{pushout}
  Denote by $\sim$ the equivalence relation $x \sim f(x)$ and set $\widetilde{M(f)} = M(f)/\sim$. The square
  \begin{equation}
  \begin{tikzcd}
  \widetilde{M(f)} & X\arrow{l}{\widetilde{i_X} = i_X/\sim} \\
  Y\arrow{u}{i_Y} & X\arrow{l}{f}\arrow{u}{\operatorname{Id}}\
  \end{tikzcd}
  \end{equation}
  is cocartesian in $Pos$.
\end{proposition}

\begin{pf}
  Consider arbitrary $Z$ such that the following diagram commutes.
  \begin{equation}
  \begin{tikzcd}
  Z & X\arrow{l}{\widetilde{i_X}} \\
  Y\arrow{u}{i_Y} & X\arrow{l}{f}\arrow{u}{\operatorname{Id}}\
  \end{tikzcd}
  \end{equation}
  
  Maps $\widetilde{i_X}$ and $i_Y$ preserve order, thus orders $R_X$ and $R_Y$ are embedded to $R_Z$. The order relation $R_{\widetilde{M(f)}}$ is a union of $R_X$ and $R_Y$ because $R_f$ is trivialized by the quotient. Hence $R_{\widetilde{M(f)}} \subset R_Z$ and there exists an unique morphism $\widetilde{M(f)} \to Z$. This morphism is an embedding into an extension.
\end{pf}
...
Note that the standard construction of a cocartesian square cannot be used instead of the mapping cylinder.
...
\begin{example}
  Let $f = (\ldots,f_n,\ldots)$ be a map of persistence posets $(X, \phi)$ and $(Y, \psi)$. Then contracted mapping cylinders $\widetilde{M(f_j)}$ form a persistence poset $\widetilde{M(f)}$ with structure maps arising from the universal property of cocartesian squares and structure maps of $X$ and $Y$.\\

  To be explicit, consider the following diagram in $Pos$ with $\widetilde{i_X}$, $j_Y$ taken from Proposition \ref{pushout}.\\
  \begin{equation}
  \begin{tikzcd}
  X_{j}\arrow{r}{{\widetilde{i_X}}_j}\arrow{d}{\phi_{i_X}}\arrow[bend left=30,swap]{rr}{f_j} & \widetilde{M(f_j)}\arrow{d}{\zeta_j} & Y_{j}\arrow{l}{{i_Y}_j}\arrow{d}{\psi_{i_Y}} \\
  X_{j+1}\arrow{r}{{i_X}_{j+1}}\arrow[bend right=30,swap]{rr}{f_{j+1}} & \widetilde{M(f_{j+1})} & Y_{j+1}\arrow{l}{{i_Y}_{j+1}} \\
  \end{tikzcd}
  \end{equation}\\

  The existence of order-preserving maps $\zeta_j$ is guaranteed by the universal property of cocartesian squares, they are set to be structure maps of $\widetilde{M(f)}$.\\
\end{example}  

\begin{definition}
  Consider the series of mapping cylinders of posets $M(f_i)$. We can define the structure maps $\chi$ on this series as $\phi$ on $X$ and $\psi$ on $Y$. They preserve orders $R_{f_i}$ by definition of structure maps. The persistence poset $(M(f), \chi_j)$ is a \define{mapping cylinder of a map of persistence posets}. We also have canonical inclusions $i_X$ and $i_Y$ arising from the same diagram.
\end{definition}

Note that the mapping cylinder is not a colimit.\\
