...
\begin{theorem} \textbf{Approximate Quillen-McCord theorem, draft statement}\\
  Assume $X, Y$ are persistent posets of finite type, $f : X \to Y$ is order-preserving map, $R$ is a field.\\
...
\begin{pf} ~ \par
 ...
 Application of Mayer-Vietoris long exact sequence to covering from Remark 6 and Proposition 11 yields the lemma.
\end{pf}
...
Let $\phi$ be structure maps of $X$. There is one case there construction of structure maps $\psi$ of $Y$ is nontrivial: image of $\phi_{i-1}$ of some $a \in Y_{i-1}$ is outside of $Y_{i}$. Then we set $\psi(a)$ to be maximal element of $Y_{i}$ such that it is not greater than $\phi(a)$.\\
