...
We expect reader to be familiar with several notions which frequently appear in introductory courses in homological and commutative algebra: chain complex, chain map, filtration, category, functor, graded ring.\\
...
Example of a persistent module is given by homology of persistence complex $C_{\star}$ (\define{persistent homology}). $H_i^j(C_{\star}) = H_i(C_{\star{j}})$, maps $\phi_j$ are induced by $f_i$.\\
